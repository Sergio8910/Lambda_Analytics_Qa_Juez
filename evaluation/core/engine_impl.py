...
def _translate_reason(reason: str) -> str:
    ...
    # Pasamos la razón tal cual — ya no descartamos el texto en inglés.
    # Con los nuevos criterios GEval en español, las razones deberían llegar
    # en español. Si siguen en inglés (fallback de deepeval), al menos el
    # texto original es útil para diagnóstico.
    if _english_ratio(texto) > 0.2:
        return texto  # devolver original en inglés, no descartar
    replacements = [
    ...
    # Si tras los reemplazos aún hay señales de inglés, devolver el texto
    # tal cual (parcialmente traducido) — mejor que descartar la razón.
    english_markers = re.compile(
        r"\b(the|because|fails?|does|response|output|question|address|irrelevant|score|english|spanish|and|or|with|"
        r"without|include|includes|included|directly|request|instruction|instructions|respond|relevant|not|information|"
        r"aligned|missing|format|summary|content|context)\b",
        re.IGNORECASE,
    )
    if english_markers.search(texto) or _english_ratio(texto) > 0.2:
        return sanitize_encoding(reason)  # devolver original, no descartar
    return texto
