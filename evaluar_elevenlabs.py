...
try:
    ...
    from evaluation.contracts import RunnerResult
    from evaluation.report_models import EvaluationSpec, MetricSpec, TestCase
    from evaluation.adapters.elevenlabs_adapter import llamar_agente
    from evaluation.adapters.tools_runner import crear_runner_con_tools_reales
    from evaluation.adapters.voice_runner import crear_runner_voz_real
    HAS_JUEZ = True
except Exception:
    HAS_JUEZ = False
...
try:
    from evaluation.contra_agente.generator import generar_batch as _ca_generar_batch
    from evaluation.contra_agente.pool import ejecutar_batch as _ca_ejecutar_batch
    from evaluation.contra_agente.evaluator import TurnEvaluator as _TurnEvaluator
    from evaluation.contra_agente.reporter import generar_reporte_batch as _ca_reporter
    from evaluation.contra_agente.reporter import generar_json_batch as _ca_json
    from evaluation.contra_agente.models import BatchResult as _BatchResult
...
class ElevenLabsAnalyzer:
    ...
    def _problemas(self) -> List[Dict]:
        ...
        if tools_usuario and prompt.get("completo"):
            from evaluation.static_checks import check_tool_prompt_alignment
...
def generar_reporte(analisis: Dict, gpt: Dict, agent_name: str, agent_id: str, juez_report=None, sugerencias: str = "", lineas_extra: Optional[Dict] = None, scores_precalculados: Optional[Dict] = None) -> str:
    ...
    if fantasma:
        ...
        for p in fantasma:
            m = re.search(r"menciona '([^']+)'", p.get("descripcion", ""))
...
try:
    from evaluation.utils.review import (
...
def ejecutar_contra_agente(
    ...
    def _adapter_factory(adapter_type: str, _agent_id: str):
        if adapter_type == "n8n":
            from evaluation.contra_agente.adapters.n8n import N8nAdapter
...
def _ejecutar_branch_con_descubrimiento_n8n(
    ...
    """
    try:
        from api.runner import run_elevenlabs_single
    except Exception as e:
...
def main():
    ...
    if agent_id.startswith("agtbrch_"):
        try:
            from api.elevenlabs_discovery import resolve_branch
        except Exception as e:
    ...
    from evaluation.contra_agente.pool import ejecutar_batch
    from evaluation.contra_agente.reporter import generar_reporte_batch, generar_json_batch
    from evaluation.contra_agente.adapters.elevenlabs import ElevenLabsAdapter
    ...
    if getattr(args, "ingest_conversations", None):
        try:
            from evaluation.utils.ingester import ingestar_conversaciones_reales
    ...
    from evaluation.recommendations import generar_recomendaciones
